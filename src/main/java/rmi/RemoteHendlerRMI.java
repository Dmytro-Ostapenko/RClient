...
public class RemoteHendlerRMI implements Closeable {
    ...
    private void process(String stringPathRandFile, String stringPathSortFile) {
        ...
        Compute.ShellSort shellSort = null;
        try {
            shellSort = new Compute.ShellSort(fileSortNum.getName(), fileRandomNum);
        ...
        try {
            bytes = proxy.executeTask(shellSort);
        } catch (RemoteException e) {
        ...
        try {
            long timeProcesMethod = proxy.timeProcesMethod(shellSort);
            responseInfo = new String("Sorted time by Shell algoritm = " + timeProcesMethod / 1000000 + " msec.");
        } catch (RemoteException e) {
            responseInfo = new String("There was an server error processing!");
        }
    }
